// JobProof Pure Vanilla JavaScript Application Module

document.addEventListener('DOMContentLoaded', () => {

  // State
  let currentUser = JSON.parse(localStorage.getItem('jobproof_user')) || null;
  ...
  function checkAuth() {
    if (currentUser) {
      ...
      if (userDisplayName) userDisplayName.textContent = currentUser.name || 'Cooper Curtis';
      renderJobs();
  ...
  function doLogin(name, email, role) {
    ...
    localStorage.setItem('jobproof_user', JSON.stringify(currentUser));
    checkAuth();
  }
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      navItems.forEach(nav => {
        nav.classList.remove('active', 'bg-blue-600', 'text-white', 'shadow-md');
  ...
  function renderJobs() {
    ...
    jobsList.innerHTML = sampleJobs.map(job => `
      <div class="job-card p-4 rounded-2xl bg-slate-800/60 border border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
  ...
  document.getElementById('post-job-form')?.addEventListener('submit', (e) => {
    ...
    const company = document.getElementById('post-company-name').value;
    const applyUrl = document.getElementById('post-apply-url').value;
    ...
    renderJobs();
    alert('Job posted & verified with 96% Trust Score!');
